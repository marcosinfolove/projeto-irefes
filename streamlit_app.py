...
# Estilo personalizado de alto contraste
st.markdown("""
    <style>
...
st.markdown("Monitoramento de Performance com Coleta Assistida por I.A.")
...
if not df.empty:
    # --- BARRA LATERAL (FOCO EM ACESSIBILIDADE VIA PROMPT) ---
    ...
    st.sidebar.divider()
    st.sidebar.markdown("""
    ### 🤖 Coleta Assistida (I.A. Workspace)
    **O jeito mais fácil para atletas PcD:**
    
    1. No Google Sheets, abra o painel do **Gemini/Assistente** à direita.
    2. Clique no campo de chat (campo único e grande).
    3. Use **Windows + H** para ditar comandos como: 
       *"Adicione uma linha para o atleta João com Sprint 8.5 e Arremesso 4.2"*.
    4. O sistema insere os dados automaticamente sem você precisar navegar entre células!
    """)
    ...
    if atleta_selecionado == "Todos":
        ...
        # VISÃO INDIVIDUAL
        st.subheader(f"📊 Evolução: {atleta_selecionado}")
        ...
        st.divider()
        st.markdown("### Comparativo Individual vs Equipe")
